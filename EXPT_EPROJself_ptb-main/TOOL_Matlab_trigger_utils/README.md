...
With the current setup, the latency will be approximately 70-80ms (time taken since detecting the trigger 
pulse to releasing control to subsequent code in the calling script). Most of this time is spent in a while-loop
cleanup that incorporates KbCheck(-1) statements with a restricted keyset (to only check Esc) to allow
exiting out of the while-loop. If you don't care about being able to exit out of the "waiting for trigger"
state and are happy to `Ctrl+C` & `sca` your way out, you can do away with another 60-70ms of latency
with even leaner code. The leaner code is implemented in a [separate script](https://github.mit.edu/EvLab/TOOL_Matlab_trigger_utils/blob/main/wait_for_trigger_kbqueue_lean.m).
